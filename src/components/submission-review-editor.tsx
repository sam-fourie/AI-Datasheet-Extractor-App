...
export function SubmissionReviewEditor({
  ...
  const normalizedDraft = normalizeSubmissionReview(
    submission.extraction,
    reviewDraft,
  );
  const reviewDecisionCounts = countReviewDecisions(normalizedDraft);
  const draftReviewStatus = getDerivedReviewStatus(reviewDecisionCounts.pending);
  const measurementReviewMap = new Map(
    reviewDraft.measurements.map((entry) => [entry.field, entry] as const),
  );
  const pinReviewMap = new Map(
    reviewDraft.pins.map((entry) => [entry.pinIndex, entry] as const),
  );
  ...
  const heading =
    mode === "live"
      ? {
          description:
            "The extraction has already been persisted. Review each row inline, add reviewer notes, and save the latest human corrections here.",
  ...
  return (
    <div className="space-y-6 pb-44 md:pb-36">
      ...
      <div className="space-y-6">
        <Card className="space-y-5">
          <div className="space-y-2">
            ...
            <h3 className="text-2xl">Capture reviewer context</h3>
            <p className="text-sm leading-6 text-text-muted">
              Submission review status is derived automatically from the package,
              measurement, and pin decisions below.
            </p>
          </div>
        ...
        <Card className="space-y-5">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              ...
              <h3 className="text-2xl">Confirm or correct the package variant</h3>
            </div>
            <span
              className={[
                "rounded-pill border px-3 py-1.5 text-xs font-medium uppercase tracking-[0.16em]",
                badgeToneClassNames[
                  getDecisionTone(reviewDraft.packageSelection.status)
                ],
              ].join(" ")}
            >
              {
                decisionOptions.find(
                  (option) => option.value === reviewDraft.packageSelection.status,
                )?.label
              }
            </span>
          </div>
          ...
          <div className="rounded-control border border-border bg-surface p-5">
                ...
                <div>
                  ...
                  <div className="mt-3">
                    <DecisionPillGroup
                      ...
                      status={reviewDraft.packageSelection.status}
                    />
                  </div>
                </div>
              </div>
            ...
            {reviewDraft.packageSelection.status === "corrected" ? (
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    >
                      <TextField
                        ...
                        value={
                          reviewDraft.packageSelection.correctedSelectedPackage ?? ""
                        }
                      />
                    </Field>
                    ...
                    >
                      <TextField
                        ...
                        value={reviewDraft.packageSelection.correctionNote ?? ""}
                      />
                    </Field>
                  </div>
                </div>
      ...
      <Card className="space-y-5">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            ...
            <h3 className="text-2xl">Confirm or correct extracted measurements</h3>
          </div>
        ...
        <div className="hidden overflow-hidden rounded-control border border-border bg-surface lg:block">
          <table className="min-w-full table-fixed border-collapse">
            <colgroup>
              {measurementDesktopColumnWidths.map((width, index) => (
      ...
      <Card className="space-y-5">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            ...
            <h3 className="text-2xl">Confirm or correct extracted pin rows</h3>
          </div>
          <span className="rounded-pill border border-border bg-surface-muted px-3 py-1.5 text-xs font-medium uppercase tracking-[0.16em] text-text-muted">
        ...
        {submission.extraction.pinRows.length > 0 ? (
          <>
            <div className="hidden overflow-hidden rounded-control border border-border bg-surface lg:block">
              <table className="min-w-full table-fixed border-collapse">
                <colgroup>
                  {pinDesktopColumnWidths.map((width, index) => (
