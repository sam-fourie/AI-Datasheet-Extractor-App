...
- `src/app/`: routes. The list page is in the `submissions/(list)/` route group so its loading state never wraps the review route. Route handlers are under `src/app/api/`.
